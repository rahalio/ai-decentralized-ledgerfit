...
import { DOMAIN_PREFIX_MAP, isValidDomainId } from '@ledgerfit/core';
import { ulid } from 'ulid';
import type { IdGeneratorService } from '@ledgerfit/services/_shared';
...
export class DefaultIdGeneratorService implements IdGeneratorService {
  ...
  ldgId(): string {
    return generateIdWithPrefix(DOMAIN_PREFIX_MAP.ledgers);
  }
  trnId(): string {
    return generateIdWithPrefix(DOMAIN_PREFIX_MAP.training);
  }
  mdlId(): string {
    return generateIdWithPrefix(DOMAIN_PREFIX_MAP.models);
  }
  trgId(): string {
    return generateIdWithPrefix(DOMAIN_PREFIX_MAP.triggers);
  }
  prvId(): string {
    return generateIdWithPrefix(DOMAIN_PREFIX_MAP.provenance);
  }
  invId(): string {
    return generateIdWithPrefix(DOMAIN_PREFIX_MAP.investigation);
  }
  govId(): string {
